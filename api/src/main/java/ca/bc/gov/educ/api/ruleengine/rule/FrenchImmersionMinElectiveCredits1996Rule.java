    ...
    public RuleProcessorData fire() {
        ...
        while (studentCourseIterator2.hasNext()) {            
            ...
            for(OptionalProgramRequirement pR:gradOptionalProgramMinCreditElectiveRulesMatch) {
                if(pR.getOptionalProgramRequirementCode().getRequiredLevel() != null && pR.getOptionalProgramRequirementCode().getRequiredLevel().trim().compareTo("11 or 12") == 0 && sc.getLanguage() != null && sc.getLanguage().toUpperCase().startsWith("F") && (sc.getCourseLevel().trim().contains("11") || sc.getCourseLevel().trim().contains("12"))) {
